...
import gradient from "gradient-string"; // Gradient colors
...
export function log(message, type = "info") {
  const infoGradient = gradient(["#FF69B4", "#FFB6C1", "#FFFFFF"]);
  const loadingGradient = gradient(["#FF0000", "#FF4444", "#FFFFFF"]);
  ...
  const hasAnsi = typeof message === "string" && message.includes("\x1b[");
  const isLoading = !hasAnsi && (message.toLowerCase().includes("loading") || message.toLowerCase().includes("loaded"));
  const styledMessage = hasAnsi ? message : isLoading ? loadingGradient(message) : infoGradient(message);
  console.log(`${prefix} ${styledMessage}`);
}
