        ...
        try {
            ...
            switch (subcommand) {
                ...
                case 'setapplicationid':
                    return await this.setApplicationId(client, message, args.slice(1));
...
> • \`setApplicationId <id>\` - Set your own Discord Application ID (required for RPC to actually show)
...
>
> Examples:**
> • \`${client.prefix}rpc setApplicationId 1306468377539379241\`
...
> • \`${client.prefix}rpc setStartTimestamp ${Date.now()}\``;
    ...
    getUsage() {
        return `Usage: \`+rpc <enable|disable|setApplicationId|setType|setURL|setState|setName|setDetails|setParty|setStartTimestamp|setEndTimestamp|setLargeImage|setLargeText|setSmallImage|setSmallText|addButton|clearButtons|view|reset>\``;
    },
    ...
    async setApplicationId(client, message, args) {
        if (!args[0]) {
            return message.channel.send("> Please specify a valid Discord Application ID.**\nGet one at https://discord.com/developers/applications → New Application → General Information → Application ID");
        }

        const appId = args[0];

        if (!/^\d{15,21}$/.test(appId)) {
            return message.channel.send("> Invalid Application ID.** Must be a numeric Discord application ID (15-21 digits).");
        }

        const config = client.rpcManager.getCurrentConfig() || this.getDefaultConfig();
        config.rpc.application_id = appId;

        client.rpcManager.updateConfig(config);
        client.rpcManager.clearAssetCache();

        const success = await client.rpcManager.updatePresence(client);
        if (success) {
            return message.channel.send(`> Application ID set to: \`${appId}\`**`);
        }
